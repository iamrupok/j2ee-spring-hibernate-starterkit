    ...
    $.fn.fcbkcomplete = function(opt){
        return this.each(function(){
            ...
            function createFCBK(){
                ...
                complete.append('<div class="default" id="hitsText'+options.itemindex+'">' + options.complete_text + "</div>");
            ...
            function (event, data) {
                ...
                if (item.length) {
                    removeItem(item);
				/*Checking for a tabbed Lock*/
				if(document.getElementById("producthidden"+selectedIndex))
					{
                    if(document.getElementById("producthidden"+selectedIndex).value=="true")
					{			

					if(document.getElementById("selectedTabbId"+selectedIndex).value=="")
					{
					var tabFlag=false;
					var specificationTabb=document.getElementById("availabeTabb"+selectedIndex).value;
					var specificationTabbIdList=document.getElementById("availabeTabbIdList"+selectedIndex).value;

					if(specificationTabb!="")
						{
						    
							var tabbArray = specificationTabb.split( "," );
							var tabbIdArray = specificationTabbIdList.split( "," );
			
							for ( var i = 0; i < tabbArray.length; i++ )
								{
								    if(tabFlag==false)
									{
									
									if(data.value==tabbIdArray[i])
									{
											tabFlag=true;
											$("#selectedTabbId"+selectedIndex).val("");
									}
				...
				$("#maininput_"+selectedIndex).click();
...
//			   //console.log("^^^^^^^");
							...
							for ( var i = 0; i < tabbArray.length; i++ )
								{
									...
									if(item.attr("rel")==tabbIdArray[i])
											{
											tabFlag=true;
											$("#selectedTabbId"+selectedIndex).val("");
											}
            ...
            function addInput(focusme){
                ...
                input.keypress(function(event){
                    ...
                    //auto expand input							
                    input.attr("size", input.val().length + 57);
                });
...
//					alert(event.keyCode);
				...
				//For Enter Key				 
				 if (event.keyCode == 13) {
                    ...
                    if(document.getElementById("producthidden"+selectedIndex).value=="true")
					{		                         
						if(is_int(etext)==false)
						{
						if(jQuery.trim($("#miscellaneous"+selectedIndex).text())!="")
						{
							//alert("Append: "+$("#miscellaneous"+selectedIndex).text());
						 $("#miscellaneous"+selectedIndex).text($("#miscellaneous"+selectedIndex).text()+", "+etext);
						 }
						 else						 
						 {
						 	//alert("Not Append: "+$("#miscellaneous"+selectedIndex).text());
						 	$("#miscellaneous"+selectedIndex).text(etext);
						 }
						 $("#maininput_"+selectedIndex).val("");
							...
							for ( var i = 0; i < tabbArray.length; i++ )
								{
								    if(tabFlag==false)
									{
									var pos=(etext.toLowerCase()).indexOf(tabbArray[ i ].toLowerCase());
				  	                
										if(pos==0)
										{
											tabFlag=true;
											tabbedName=tabbArray[ i ];
											tabbedId=tabbIdArray[i];
											$("#typeAhead"+selectedIndex).trigger("addItem",[{"title": tabbedName, "value": tabbedId}]);
											document.getElementById("selectedTabbId"+selectedIndex).value=tabbedId;											
										}
								     }
...
//							//console.log("****************************************");
					...
					/*********/
					tabbId=document.getElementById("typeAhead"+document.getElementById("currentIndex").value).value;
					if(tabbId=="")
					tabbId=0
					//alert(tabbId);
                        ...
                        if (options.json_url) { 
                            if (options.cache && json_cache) {
                                ...
                                setTimeout (function() {
									...
									var productId=document.getElementById("selectedProduct"+document.getElementById("currentIndex").value).value;
//									//console.log("SI "+document.getElementById("currentIndex").value);
//									//console.log(productId);
                                    ...
                                    $.getJSON(options.json_url + ( options.json_url.indexOf("?") > -1 ? "&" : "?" ) + "tag=" + etext+"&type="+searchType+"&productId="+productId+"&tabbId="+tabbId, null, function (data) {
            ...
            function addMembers(etext, data){
                ...
                if (data[options.root] != null && data[options.root].length) {
					
                    $.each(data[options.root], function(i, val){ 
            ...
            function bindEvents(){
                ...
                maininput.keydown(function(event){
					...
					if ((event.keyCode == 38 || event.keyCode == 40) && checkFocusOn()) {
                        // Show currently selected item in textbox
                        var option = feed.children('li[class="auto-focus"]');
                        showSelectedItem("maininput_"+selectedIndex,option.text());
                    }
                });
            }
            
			function showSelectedItem(maininputId,text) {
				document.getElementById(maininputId).value=text;
                //$(".maininput").val(text);
            }
